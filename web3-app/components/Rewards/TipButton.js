...
import { buildCreatorSubPaymentTx } from "../../lib/xrplClient";

const TIP_OPTIONS = [
  { label: "0.5",  drops: 500_000 },
  { label: "1",    drops: 1_000_000 },
  { label: "5",    drops: 5_000_000 },
  { label: "10",   drops: 10_000_000 },
];
...
export default function TipButton({ recipientAddress, recipientName, compact = false }) {
  ...
  async function handleTip(drops) {
    ...
    try {
      ...
      const p = await xamanSendTx(tx, { identifier: "tc-tip", instruction: `Tip ${recipientName} ${drops / 1_000_000} XRP` });
      setPayload(p);
      ...
      <div style={{ position: "relative" }}>
        ...
        {open && (
          <div style={{
            ...
            <div style={{ fontSize: 11, color: "rgba(255,255,255,0.5)", marginBottom: 4 }}>Tip with XRP</div>
            {TIP_OPTIONS.map(o => (
              <button key={o.label} onClick={() => handleTip(o.drops)} disabled={tipping} style={{
                padding:      "8px 12px",
